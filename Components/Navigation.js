            ...
            <div className="container mx-auto px-6 py-4">
                ...
                {isMobileMenuOpen && (
                    <motion.div
                        ...
                        className="md:hidden mt-4 pb-4 border-t border-yellow-500/20"
                    >
                        {navItems.map((item) => (
                            <motion.button
                                key={item.name}
                                whileHover={{ x: 10 }}
                                onClick={() => scrollToSection(item.href)}
                                className="block w-full text-left py-2 text-gray-300 hover:text-yellow-500 transition-colors duration-300"
                            >
                                {item.name}
                            </motion.button>
                        ))}
